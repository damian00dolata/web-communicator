	...
	async function createUser() {
		...
		if (nicknameResult && emailResult && passwordResult) {
			...
			if (response.ok) {
				...
				clearFormResponse();
				addFormResponse(
					'[' + response.status + '] ' + response.statusText + ': ' + responseText.message
				);
			}
		} else {
			clearFormResponse();
			if (!nicknameResult) {
				addFormResponse(
					"Nickname must be 5-20 characters long, contain only alphanumeric characters, underscores and dots, that can't be at the beginning or the end of a nickname."
				);
			}
			if (!emailResult) {
			...
			<div class="formBox">
				<div class="inputBox">
					<input
						...
						placeholder="Nickname"
						on:keypress={onKeyPressRegister}
					/>
					<input
						type="text"
						id="email"
...
<style>
	...
	.buttonSwapLeft > .active,
	.buttonSwapRight > .active {
		color: white;
